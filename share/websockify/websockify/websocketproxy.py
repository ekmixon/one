...
"""
    ...
    def new_websocket_client(self):
        ...
        if self.server.wrap_cmd:
            ...
            msg = f"connecting to unix socket: {self.server.unix_target}"
        else:
            msg = f"connecting to: {self.server.target_host}:{self.server.target_port}"
...
class WebSocketProxy(websocket.WebSocketServer):
    ...
    def started(self):
        ...
        if self.wrap_cmd:
            ...
            dst_string = f"{self.target_host}:{self.target_port}"

        if self.token_plugin:
            msg = f"  - proxying from {self.listen_host}:{self.listen_port} to targets generated by {type(self.token_plugin).__name__}"

        else:
            msg = f"  - proxying from {self.listen_host}:{self.listen_port} to {dst_string}"
    ...
    def poll(self):
        ...
        if self.wrap_cmd and self.cmd:
            ret = self.cmd.poll()
            if ret != None:
                self.vmsg(f"Wrapped command exited (or daemon). Returned {ret}")
                self.cmd = None
...
def websockify_init():
    ...
    if opts.ssl_only and not os.path.exists(opts.cert):
        parser.error(f"SSL only and {opts.cert} not found")
    ...
    if opts.token_plugin is not None:
        if '.' not in opts.token_plugin:
            opts.token_plugin = f'websockify.token_plugins.{opts.token_plugin}'
    ...
    if opts.auth_plugin is not None:
        if '.' not in opts.auth_plugin:
            opts.auth_plugin = f'websockify.auth_plugins.{opts.auth_plugin}'
...
class LibProxyServer(ForkingMixIn, HTTPServer):
    ...
    def __init__(self, RequestHandlerClass=ProxyRequestHandler, **kwargs):
        ...
        if record := kwargs.pop('record', ''):
            self.record = os.path.abspath(record)
        self.run_once  = kwargs.pop('run_once', False)
        self.handler_id = 0

        for arg in kwargs:
            print(f"warning: option {arg} ignored when using --libserver")
